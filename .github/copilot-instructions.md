...
- **Color values exist ONLY inside `[data-agt-theme="..."]` scopes** (theme files under `wwwroot/css/themes/`). `:root` holds structural tokens only (spacing, radius, fonts, motion, z-index). This is enforced by the token-audit tests — never define a color in `:root`, a bare class, `.razor.css`, inline styles, or C# code. New colors = new theme-scoped tokens, added to EVERY family (the parity test fails otherwise).
